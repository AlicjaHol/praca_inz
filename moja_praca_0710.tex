...
\begin{table}[!h]
	\begin{center}
		\caption{Wyniki dla modelu mieszanego z uwzględnieniem wpływu kraju na wyraz wolny; zależność pomiędzy liczbą zachorowań a czasem}
...
\begin{table}[!htbp]
	\begin{center}
		\caption{Wyniki dla modelu mieszanego z uwzględnieniem wpływu kraju na wyraz wolny i przesunięcie linii regresji; zależność pomiędzy liczbą zachorowań a czasem }
...
\begin{table}[!h]
	\begin{center}
		\caption{Wyniki dla modelu wielomianowego mieszanego uwzględniającego wpływ kraju; zależność pomiędzy liczbą zachorowań a czasem}
...
\begin{table}[!h]
	\begin{center}
		\caption{Wyniki dla modelu mieszanego wielomianowego stopnia trzeciego; zależność pomiędzy liczbą zachorowań a czasem}
...
\begin{table}[!htbp]
	\begin{center}
		\caption{Wyniki dla modelu mieszanego uwzględniającego wpływ kraju na wyraz wolny; zależność pomiędzy liczbą zachorowań a liczbą wykonywanych testów}
...
\begin{table}[!h]
\begin{center}
		\caption{Wyniki dla modelu liniowego uwzględniającego wpływ kraju na parametry modelu; zależność pomiędzy liczbą zachorowań a liczbą wykonywanych testów}
...
\begin{table}[!htbp]
	\begin{center}
		\caption{Wyniki dla modelu regresji prostej po przekształceniu zmiennych; zależność pomiędzy liczbą zachorowań a oczekiwaną długością życia}
...
\begin{figure}[!h]
	...
	\caption{Zależność liczby zachorowań od siły obostrzeń opóźnionej o 7 dni; wyniki dla modelu regresji prostej po przekształceniu zmiennych}
...
\begin{table}[!h]
	\begin{center}
		\caption{Wyniki dla modelu mieszanego z losowym wyrazem wolnym; zależność pomiędzy liczbą zachorowań a siłą obostrzeń opóźnioną o 7 dni}
...
\begin{table}[!htpb]
	\begin{center}
		\caption{Wyniki dla modelu mieszanego, w którym wyraz wolny oraz $\beta_{stringency\_lag7}$ zależą od kraju; zależność pomiędzy liczbą zachorowań a siłą obostrzeń opóźnioną o 7 dni}
...
\begin{table}
	\begin{center}
			\caption{Wyniki dla modelu mieszanego z drugą potęgą zmiennej $stringensy\_lag7$; zależność pomiędzy liczbą zachorowań a siłą obostrzeń opóźnioną o 7 dni}
...
Podsumowanie modelu wygląda jak w tabeli \ref{table:mod6-log}. Współczynnik $\beta_{HDI}$ wynosi około $5.57$ i jest istotny statystycznie ($p$-value poniżej $0.001$). Wartość tego współczynnika jest dodatnia, co oznacza, że im wyższy wskaźnik rozwoju społecznego w danym kraju, tym więcej potwierdzonych przypadków COVID-19. Współczynnik determinacji wynosi około $15\%$, więc jest dość niski. Dopasowanie modelu do danych jest ukazane na rysunku \ref{fig:mod6-log}.

\newpage
\begin{table}[!htbp]
	\begin{center}
			\caption{Wyniki dla modelu regresji prostej po przekształceniu zmiennych; zależność pomiędzy liczbą zachorowań a wskaźnikiem rozwoju społecznego}
...
\begin{table}[!htbp]
	\begin{center}
		\caption{Wyniki dla modelu regresji prostej z przekształceniem logarytmicznym obu zmiennych; zależność pomiędzy liczbą zachorowań a częścią populacji \\żyjącą w skrajnej biedzie}
...
\bibitem{epidemie} Kosiński R., Grabowski A., \emph{Modelowanie epidemii. Jak epidemie się szerzą i jak im przeciwdziałać}, Bezpieczeństwo Pracy 05/2009, str. 3-5, \url{http://www.bhportal.pl/32543} (dostęp: 21.01.2021)
\newpage
...
\bibitem{wykres} Pogotowie Statystyczne, \emph{Analizy statystyczne bez tajemnic, czyli dziwne wyniki analizy korelacji w podziale na podzbiory - cz.1}  \texttt{https://pogotowiestatystyczne.pl/analizy-statystyczne-bez-tajemnic-czyli-\\dziwne-wyniki-analizy-korelacji-podziale-podzbiory-cz-1/} (dostęp: 21.01.2021)
