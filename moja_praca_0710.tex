...
Na rysunku \ref{fig:total_cases_countries} jest pokazana zależność pomiędzy liczbą zachorowań a czasem (gdzie czas rozumiemy jako kolejne dni trwania epidemii).

\begin{figure}[!ht]
	\centering
	\includegraphics[width=\linewidth]{rys/total_cases_countries.png}
	\caption{Wykres przedstawiający zależność liczby zachorowań na milion mieszkańców w zależności od czasu w podziale na kraje}
...
Zarówno wyraz wolny, jak i współczynnik przy zmiennej \texttt{time}, są istotne statystycznie ($p$-value poniżej $0.001$). Dodatkowo, $\beta_{time}$ wynosi $37.31$, jest dodatni, więc upływający czas sprawia, że liczba zachorowań rośnie.
...
\begin{table}[!htbp]
	\begin{center}
		\begin{tabular}{l c}
			...
			\multicolumn{2}{l}{\scriptsize{$^{***}p<0.001$; $^{**}p<0.01$; $^{*}p<0.05$}}
		\end{tabular}
		\caption{Wyniki dla modelu 1 z uwzględnieniem wpływu kraju na wyraz wolny i przesunięcie linii regresji }
...
\begin{figure}[!ht]
	...
	\zrodlo{Opracowanie własne}
\end{figure}
\noindent Na rysunku \ref{fig:mod1_slope_predict} można zaobserwować, jak różnią się tendencje rozwojowe pandemii w poszczególnych krajach.
...
Wracając do rysunku \ref{fig:total_cases_countries}, można podejrzewać, że do danych będzie dobrze pasował model mieszany, w którym efekt stały czasu jest opisany wielomianem trzeciego stopnia, czyli
...
Wszystkie trzy potęgi zmiennej $time$ są istotne statystycznie ($p$-value poniżej $0.001$), w dodatku przy każdej z nich współczynnik jest dodatni. Wykresy dopasowane na podstawie tego modelu są przedstawione na rysunku \ref{fig:mod1_poly_predict}.
\newpage
\begin{figure}[!h]
	\centering
...
\subsection{Model 2: zależność między liczbą zachorowań a liczbą wykonywanych testów na COVID-19}

Hipoteza 2: Liczba wykonywanych testów na COVID-19 ma związek z liczbą zachorowań.

Na rysunku \ref{fig:tests} przedstawiona jest zależność liczby zachorowań od liczby wykonywanych testów.
\newpage
...
Proste regresji dopasowane z tego modelu są widoczne na wykresie \ref{fig:mod2-slope}.
\newpage
\begin{figure}[!h]
	\centering
...
\subsection{Model 5: zależność między liczbą zachorowań a siłą obostrzeń}
...
\begin{figure}[!h]
	...
	\caption{Proste regresji dopasowane do zależności między pierwszą i drugą potęgą opóźnionej o 7 dni siły obostrzeń a liczbą zachorowań }
...
\bibitem{bootstrap} Tim Hesterberg, Shaun Monaghan, David S. Moore, Ashley Clipson, Rachel Epstein, \emph{Bootstrap Methods and Permutation Tests. Companion Chapter 18 to the Practice of Business Statistics}, W. H. Freeman and Company, New York, 2003
...
\bibitem{covid-19} Jerzy Duszyński, Aneta Afelt, Anna Ochab-Marcinek,
...
\bibitem{cholesky} Taboga, Marco (2017). \emph{Cholesky decomposition, Lectures on matrix algebra.} \url{https://www.statlect.com/matrix-algebra/Cholesky-decomposition} (dostęp: 09.01.2021)

\bibitem{insurance} Piet de Jong, Gillian Z. Heller, \emph{Generalized Linear Models for Insurance Data}, Cambridge University Press, New York, 2008

\bibitem{complex} Lang Wu, \emph{Mixed Effects Models for Complex Data}, University of British Columbia, Vancouver, Canada, 2010

\bibitem{zuur} Alain F. Zuur, Elena N. Ieno, Neil J. Walker,
Anatoly A. Saveliev, Graham M. Smith, \emph{Mixed Effects Models and Extensions in Ecology with R}, Springer, New York 2009

\bibitem{owid} \url{https://ourworldindata.org/coronavirus} (dsotęp: 30.11.2020)
...
\bibitem{lme4} \url{https://cran.r-project.org/web/packages/lme4/index.html} (dostęp: 03.01.2021)

\bibitem{lmerTest} \url{https://cran.r-project.org/web/packages/lmerTest/index.html} (dostęp: 03.01.2021)

\bibitem{lm} \url{https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/lm} (dostęp: 03.01.2021)

\bibitem{powerTransform} \url{https://www.rdocumentation.org/packages/car/versions/3.0-10/topics/powerTransform} (dostęp: 03.01.2021)


\bibitem{pulp} \url{https://www.rdocumentation.org/packages/faraway/versions/1.0.7/topics/pulp} (dostęp: 04.01.2021)



\bibitem{rozrzut} \url{https://www.naukowiec.org/wiedza/statystyka/interpretacja-wykresow-rozrzutu_769.html} (dostęp: 07.01.2021)



\bibitem{boxcox} \url{https://pl.wikipedia.org/wiki/Przekszta\%C5\%82cenie\_Boxa-Coxa} (dostęp: 07.01.2021)

\bibitem{r2} \url{https://pl.wikipedia.org/wiki/Wsp\%C3\%B3\%C5\%82czynnik\_determinacji} (dostęp: 08.01.2021)

\bibitem{skorygowany} \url{https://pl.wikipedia.org/wiki/Model\_statystyczny#Skorygowany\_wsp\%C3\%B3\%C5\%82czynnik\_determinacji} (dostęp: 08.01.2021)

\bibitem{narazenie} \url{https://www.medonet.pl/koronawirus-pytania-i-odpowiedzi/sars-cov-2,kto-jest-w-najwiekszej-grupie-ryzyka-koronawirusa-,artykul,48398767.html} (dostęp: 09.01.2021)



\bibitem{rzadka} \url{https://pl.wikipedia.org/wiki/Macierz_rzadka} (dostęp: 08.01.2021)

\bibitem{bayes} \url{https://pl.wikipedia.org/wiki/Twierdzenie_Bayesa} (dostęp: 08.01.2021)



\bibitem{codebook} \url{https://github.com/owid/covid-19-data/blob/master/public/data/owid-covid-codebook.csv} (dostęp: 09.01.2021)

\bibitem{hdi} \url{https://pl.wikipedia.org/wiki/Wska\%C5\%BAnik\_rozwoju\_spo\%C5\%82ecznego} (dostęp: 09.01.2021)
